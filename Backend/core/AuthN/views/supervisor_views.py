...
class SupervisorRegisterView(APIView):
    def post(self, request):
        ...
        try:
            # Decode the token and get the Organization user object
            user = decode_jwt_token(auth_header, ["Admin"])  # Adjust based on your decoding logic

            if not isinstance(user, Admin):
                return Response({"error": "Invalid user type."}, status=status.HTTP_400_BAD_REQUEST)

            # Check if the user has permission (Admin should be able to register Supervisors)
            data = request.data
            data['admin'] = user.id  # Associate supervisor with the current admin

            # Check for duplicate email or username
            if Supervisor.objects.filter(email=data.get('email')).exists():
                return Response({"error": "Email already exists."}, status=status.HTTP_400_BAD_REQUEST)
            if Supervisor.objects.filter(username=data.get('username')).exists():
                return Response({"error": "Username already exists."}, status=status.HTTP_400_BAD_REQUEST)

            # Create a Supervisor using the provided data
...
class SupervisorLoginView(APIView):
    def post(self, request):
        ...
        try:
            ...
            if supervisor.is_active and check_password(password, supervisor.password):
