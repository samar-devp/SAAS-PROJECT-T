...
from .serializers import ExpenseCategorySerializer, ExpenseCategoryUpdateSerializer
from AuthN.models import AdminProfile

class ExpenseCategoryAPIView(APIView):
    def get(self, request, admin_id, pk=None):
        if pk:
            obj = get_object_or_404(ExpenseCategory, admin__id=admin_id, id=pk, is_active=True)
            return Response(ExpenseCategorySerializer(obj).data)
        categories = ExpenseCategory.objects.filter(admin__id=admin_id, is_active=True)
    ...
    def put(self, request, admin_id, pk):
        obj = get_object_or_404(ExpenseCategory, admin__id=admin_id, id=pk)
        serializer = ExpenseCategoryUpdateSerializer(obj, data=request.data)
        if serializer.is_valid():
            serializer.save()
